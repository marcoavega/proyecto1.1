<?php
require("agregar-lecciones-validacion.php");
...
<html lang="es-MX">
   ...
   <head>
      ...
      <script src="../../../javascript/validar-agregar-lecciones.js"></script>
   ...
   <body>
      ...
      <div class="contenedor-filas-columnas">
         ...
         <div class="container">
            <h2 class="h2formulario">Captura de datos de lección</h2>
            <form name="validar" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" onsubmit="return validarLecciones()" class="formulario">
               <label for="comentar" class="labelform">Favor de ingresar los datos de lección:</label>
               <div class="columna1">
                  <label for="fname">Selección el trimestre:</label>
               </div>
               <div class="columna2">
                  <?php
                     //sentencia SQL y la ejecutamos para obtener las lecciones
                     $sSQL   = "Select datos_trimestre From escuelas_trimestres";
                     $result = mysqli_query($conexion, $sSQL);
                     echo '<select name="datos_trimestre" class="input">';
                     //menu desplegable con las lecciones
                     while ($row = mysqli_fetch_array($result)) {
                         echo '<option>' . $row["datos_trimestre"];
                     }
                     ?>
                  </select>
                  <span class="input-100"><?php echo $datosErr;?></span>
               </div>
               <!--formulario para ingresar título de lecciones-->
               <div class="columna1">
                  <label for="comentario">Ingrese el título de la lección:</label>
               </div>
               <div class="columna2">
               ...
               <div class="boton_submit">
                  <input type="submit" value="Guardar datos lección" class="btn-enviar">
