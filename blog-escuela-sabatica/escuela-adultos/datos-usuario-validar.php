...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["usuarioanterior"])) {
        ...
        if (!preg_match("/^[a-zA-Z áéíóúÁÉÍÓÚñÑ ]*$/", $usuarioanterior)) {
            $usuarioanteriorErr = "Sólo se permiten letras y espacios en blanco.";
        } else {//
            
            if (strlen($usuarioanterior) > 26) {
                $usuarioanteriorErr = "el nombre debe de ser menor a 26 caracteres";
            } else {//
                if (empty($_POST["usuarionuevo"])) {
                    ...
                    if (!preg_match("/^[a-zA-Z ]*$/", $usuarionuevo)) {
                        $usuarionuevoErr = "Sólo se permiten letras y espacios en blanco.";
                    }
                    if (strlen($usuarionuevo) > 26) {
                        $usuarionuevoErr = "El nombre de usuario deben de ser menor a 26 caracteres";
                    } else {//
                        if (empty($_POST["contrasenanueva"])) {
                            ...
                            if (empty($_POST["contrasenanueva2"])) {
                                ...
                                if ($contrasenanueva == $contrasenanueva2 && $usuarioanterior == $_SESSION['nombre_usuario']) {
                                 ...
                                 $sql                   = "UPDATE usuario_blog_adultos SET nombre_usuario='$usuarionuevo', contrasena='$contrasena_encriptada' WHERE nombre_usuario='$usuarioanterior'";
                                 //verificamos que no exista ya ese usuario
                                 $verificar_usuario     = mysqli_query($conexion, "SELECT * FROM usuario_blog_adultos WHERE nombre_usuario = '$usuarionuevo'");
                                 ...
                                 if (mysqli_query($conexion, $sql)) {
                                     echo '<script>
                                        ...
                                        window.location.replace("login-blog-adultos.php");
                                        </script>';
                                 } else {
