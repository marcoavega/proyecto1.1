...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["usuario"])) {
        ...
        if (!preg_match("/^[a-z A-Z áéíóúÁÉÍÓÚ ñÑ üÜ]*$/", $usuario)) {
    ...
    if (empty($_POST["contrasena"])) {
        ...
        if(strlen($contrasena) < 4 || strlen($contrasena) > 255){
    ...
    if ($captcha != "" && $_POST['captcha'] == $_SESSION['captcha']) {
        ...
        if ($usuario != "" && $filas['nombre_usuario'] == $usuario) {
            if ($filas > 0 && password_verify($contrasena, $filas["contrasena"])) {
                ...
                $id                         = $filas["id_usuario_administrador"];
                header("location:principal-sis-admin.php"); 
            } else {
                if ($contrasena != "") {
