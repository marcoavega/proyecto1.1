...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["usuario"])) {
        ...
        if (!preg_match("/^[a-zA-Z áéíóúÁÉÍÓÚñÑ]*$/", $usuario)) {
    ...
    if (empty($_POST["cantidad"])) {
        ...
        $cantidad = test_input($_POST["cantidad"] * -1);
        // check if name only contains letters and whitespace
    ...
    if (empty($usuario) || empty($cantidad) || empty($descripcion) || empty($tipo) || empty($numero) || empty($id) || !preg_match("/^[a-zA-Z áéíóúÁÉÍÓÚñÑ]*$/", $usuario) || !preg_match("/^[ 0-9 . -0-9 ]*$/", $cantidad) || !preg_match('/^[a-z A-Z á é í ó ú Á É Í Ó Ú ñ Ñ 0-9 , . ( ) “ ” " " : ¿ ? ! ¡ º ª - ; \-\s ]*$/', $descripcion) || !preg_match('/^[a-z A-Z á é í ó ú Á É Í Ó Ú ñ Ñ 0-9 , . ( ) “ ” " " : ¿ ? ! ¡ º ª - ; \-\s ]*$/', $tipo) || !preg_match('/^[a-z A-Z á é í ó ú Á É Í Ó Ú ñ Ñ 0-9 , . ( ) “ ” " " : ¿ ? ! ¡ º ª - ; \-\s ]*$/', $numero) || !preg_match("/^[ 0-9 ]*$/", $id) ){
        $envioErr = "Error al guardar datos, existen campos vaciós o tipos de datos incorrectos";
    }else{

        $miconsulta    = "INSERT INTO detalle_salida_efectivo (nombre_usuario, cantidad, descripcion, tipo_comprobante, numero_comprobante, fecha_salida, id_usuario_administrador) VALUES ('" . $usuario . "', '" . $cantidad . "', '" . $descripcion . "', '" . $tipo . "', '" . $numero . "', NOW(),'" . $id . "')";
